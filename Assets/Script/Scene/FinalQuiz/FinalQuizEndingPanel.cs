...
public class FinalQuizEndingPanel : MonoBehaviour
{
    ...
    public async UniTask ShowEnding()
    {
        ...
        seq.Append(endingTitle.DOFade(0, 1f).From(1).SetEase(Ease.Linear));
        ...
        // 타워 게임 정보
        sb.Clear();
        sb.Append("<size=70>[ 천하제일 꿀밤대회 ]</size>\n\n");
        ...
        sb.Clear();
        sb.Append("<size=70>[ 연모아 새벽반 입부 시험 ]</size>\n\n");
        ...
        sb.Clear();
        sb.Append("<size=70>[ 플레이어 기록 ]</size>\n\n");
        ...
        // 개발자 소개
        sb.Clear();
        sb.Append("<size=70>[ YMA MINI GAME ]</size>\n\n");
        sb.Append($"기획 : Hanel\n");
        ...
        sb.Clear();
        sb.Append("<size=70>[ Special Thanks ]</size>\n\n");
    ...
    private async UniTask FadeOutAllCredit()
    {
        foreach (var credit in endingCredits)
        {
            credit.DOFade(0f, 1f).From(1f).SetEase(Ease.Linear);
        }
    ...
    {
        target.DOFade(0f, duration).From(1f).SetEase(Ease.Linear);
    }
}
